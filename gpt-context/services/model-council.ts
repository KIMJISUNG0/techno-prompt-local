...
export async function runModelCouncil(opts: CouncilOptions): Promise<CouncilResult> {
  ...
  if (!opts.userPrompt || opts.userPrompt.trim().length < 3) {
    return {
      ...
      timeline: [],
      proposedPatches: [],
      summaryMarkdown: '**Error: prompt too short**',
      warnings: ['prompt too short'],
    };
  }
  if (!openai)
    warnings.push(
      mockMode ? 'OPENAI_API_KEY missing -> mock fallback' : 'OPENAI_API_KEY missing (GPT steps will fail)'
    );
  if (!genAI)
    warnings.push(
      mockMode ? 'GEMINI_API_KEY missing -> mock fallback' : 'GEMINI_API_KEY missing (Gemini steps will fail)'
    );
  ...
  const requirements = await record('requirements', mReq, async () => {
    if (mockMode && !genAI)
      return mockChat({ model: 'gemini-mock', role: 'requirements', prompt: opts.userPrompt, format: 'json' });
    ...
    const resp: any = await model.generateContent(
      `요구사항을 JSON 스키마 스타일 + assumptions 필드로 요약:\n${opts.userPrompt}`
    );
  ...
  const architecture = await record('architecture', mArch, async () => {
    if (mockMode && !openai)
      return mockChat({ model: 'gpt-mock', role: 'architecture', prompt: opts.userPrompt, format: 'markdown' });
  ...
  const draft = await record('implementation-draft', mImpl, async () => {
    if (mockMode && !openai)
      return mockChat({ model: 'gpt-mock', role: 'draft', prompt: architecture, format: 'patches' });
    if (!openai) throw new Error('openai unavailable');
    const r = await openai.chat.completions.create({
      model: mImpl,
      ...
      messages: [
        {
          role: 'system',
          content: 'You write concise TypeScript patches. Use patch blocks with FILE header and PATCH delimiters.',
        },
  ...
  const review = await record('review', mReview, async () => {
    if (mockMode && !genAI)
      return mockChat({ model: 'gemini-mock', role: 'review', prompt: draft, format: 'markdown' });
    if (!genAI) throw new Error('gemini unavailable');
  ...
  const _refined = await record('refine', mRefine, async () => {
    if (mockMode && !openai)
      return mockChat({ model: 'gpt-mock', role: 'refine', prompt: draft + review, format: 'patches' });
    if (!openai) throw new Error('openai unavailable');
    const r = await openai.chat.completions.create({
      model: mRefine,
      ...
      messages: [
        {
          role: 'system',
          content: 'You merge feedback into improved patches. Output only patches + final summary section.',
        },
        {
          role: 'user',
