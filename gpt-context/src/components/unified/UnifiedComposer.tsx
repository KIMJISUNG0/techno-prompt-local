...
export default function UnifiedComposer() {
  ...
  const rec = React.useMemo(
    ...
    [intent.moods.join(','), useCase, era, intensity, durationSec]
  );
    ...
    const next = {
      ...
      sections: draft.sections.map(sec => ({
        ...
        roles: Object.fromEntries(
          Object.entries(sec.roles).map(([r, desc]) => {
  ...
  function toggleAbstractToken(catId: string, tokenId: string) {
    setAbstractTokens(s => {
      ...
      if (cur.has(tokenId)) cur.delete(tokenId);
      else cur.add(tokenId);
  ...
  function renderCategoryFlat(cat: ProgCategory) {
    ...
    const filteredTokens = cat.tokens.filter(
      tok => !abstractFilter || tok.label.toLowerCase().includes(abstractFilter.toLowerCase())
    );
    ...
    return (
      <div key={cat.id} className="space-y-1">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-slate-400">
          <span>{cat.label}</span>
          <span className="text-slate-600 lowercase">
            {picked.length}
            {cat.max ? '/' + cat.max : ''}
          </span>
        </div>
          ...
          {filteredTokens.map(tok => {
            ...
            return (
              <button
                type="button"
                key={tok.id}
                onClick={() => toggleAbstractToken(cat.id, tok.id)}
                title={tok.hint || ''}
                className={`px-2 py-[3px] rounded border text-[10px] ${on ? 'border-cyan-400 text-cyan-200 bg-cyan-600/20' : 'border-slate-600 text-slate-300 hover:border-cyan-400'}`}
              >
                {tok.label}
              </button>
            );
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2 items-center text-[10px]">
          <input
            ...
            className="px-2 py-[4px] rounded border border-slate-600 bg-slate-900/40 focus:outline-none focus:border-cyan-400 text-[10px]"
          />
          <button
            onClick={() => setAbstractMode(m => (m === 'flat' ? 'group' : 'flat'))}
          ...
          <button
            disabled={!draft}
            onClick={applyAbstractToRoles}
            className={`px-2 py-[4px] rounded border text-[10px] ${draft ? 'border-emerald-400 text-emerald-200 bg-emerald-600/10' : 'border-slate-700 text-slate-600 cursor-not-allowed'}`}
          >
            Apply → Roles
          </button>
          <button
            onClick={() => setAbstractTokens({})}
            className="px-2 py-[4px] rounded border border-slate-600 text-[10px] hover:border-amber-400"
          >
            Clear
          </button>
          <div className="text-slate-500">
            선택 토큰 수: {Object.values(abstractTokens).reduce((a, b) => a + b.length, 0)}
          </div>
        </div>
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">{cats.map(cat => renderCategoryFlat(cat))}</div>
      </div>
    );
  }
  function renderInstrumentTab() {
    return (
      <div className="space-y-3">
        {draft ? (
          <div className="text-[11px] text-slate-500">Draft roles 편집 UI 예정 (descriptor 정규화 / gap-fill)</div>
        ) : (
          <div className="text-[11px] text-slate-600">Draft 먼저 생성</div>
        )}
      </div>
    );
  }
  function renderStructureTab() {
    return (
      <div className="space-y-3">
        {draft ? (
          <div className="text-[11px] text-slate-500">Structure timeline / energy curve editor 예정</div>
        ) : (
          <div className="text-[11px] text-slate-600">Draft 먼저 생성</div>
        )}
      </div>
      ...
      <header className="flex flex-wrap items-end gap-4 justify-between">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-1">
            장르/Intent + Abstract · Instrument · Structure 통합 실험 레이아웃 – 단계적 이식.
          </p>
        </div>
        <div className="flex gap-2 text-xs">
          ...
          {draft && (
            <button onClick={() => navigator.clipboard.writeText(serialized)} className="btn">
              Copy Draft
            </button>
          )}
        </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="px-2 py-[3px] rounded border border-slate-600 text-[10px] text-slate-300 hover:border-cyan-400"
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full text-sm rounded border border-slate-700 bg-slate-900/40 px-2 py-1 focus:outline-none focus:border-cyan-400"
                >
                  {['90s', '2000s', 'modern', 'futuristic'].map(ea => (
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="number"
                  value={durationSec}
                  onChange={e => setDurationSec(Math.max(30, Number(e.target.value) || durationSec))}
                  className="w-full text-sm rounded border border-slate-700 bg-slate-900/40 px-2 py-1 focus:outline-none focus:border-cyan-400"
                />
              </div>
              <div className="text-[11px] text-slate-500 border border-slate-700 rounded p-2 bg-slate-900/30">
                ...
                <div>
                  <span className="text-slate-400">Candidates:</span> {rec.candidates.length}
                </div>
                <div>
                  <span className="text-slate-400">Issues:</span> {rec.issues.join('; ') || 'none'}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-2">
          <h3 className="text-[11px] uppercase tracking-wider text-slate-400 flex items-center gap-3">
            Genre / Hybrid Picks
            <label className="flex items-center gap-1 text-[10px] font-normal text-slate-500 cursor-pointer select-none">
              <input type="checkbox" checked={autoSeed} onChange={e => setAutoSeed(e.target.checked)} /> auto-seed
            </label>
          </h3>
          <div className="flex flex-wrap gap-2">
            {rec.candidates.slice(0, 18).map(c => {
              ...
              return (
                <button
                  key={c.genre}
                  onClick={() => toggleGenre(c.genre)}
                  className={`px-3 py-1 rounded border text-[11px] transition ${on ? 'border-cyan-400 text-cyan-200 bg-cyan-600/20' : 'border-slate-600 text-slate-300 hover:border-cyan-400'}`}
                >
                  {c.genre}
                  <span className="ml-1 text-[9px] text-slate-500">{(c.confidence * 100).toFixed(0)}%</span>
                </button>
              );
            })}
          ...
          {!autoSeed && (
            <button
              ...
              onClick={seedDraft}
              className={`mt-1 px-4 py-1.5 rounded border text-xs ${selectedGenres.length ? 'border-emerald-400 text-emerald-200 bg-emerald-600/10' : 'border-slate-700 text-slate-600 cursor-not-allowed'}`}
            >
              Seed Draft
            </button>
          )}
        ...
        <div className="flex flex-wrap gap-2">
          {(['abstract', 'instrument', 'structure'] as TabId[]).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-3 py-1 rounded border text-[11px] ${tab === t ? 'border-cyan-400 text-cyan-200 bg-cyan-600/20' : 'border-slate-600 text-slate-300 hover:border-cyan-400'}`}
            >
              {t}
            </button>
          ))}
          ...
          {expertHints ? (
            <div className="text-[10px] space-y-2 max-h-72 overflow-auto border border-slate-700 rounded p-2 bg-slate-900/30">
              ...
              {expertHints.missingRoles.length > 0 && (
                <div>
                  <span className="text-slate-400">Missing:</span> {expertHints.missingRoles.join(', ')}
                </div>
              )}
              {expertHints.underusedHighEnergy.length > 0 && (
                <div>
                  <span className="text-slate-400">Sparse HE:</span> {expertHints.underusedHighEnergy.join(', ')}
                </div>
              )}
