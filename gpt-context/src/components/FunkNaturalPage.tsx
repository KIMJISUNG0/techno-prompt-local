...
const CODEMAP: Record<string, string> = {
  'synth lead': 'synthLD',
  'talkbox lead': 'tbx',
  'wah comp guitar': 'Gwah',
  'muted chanks': 'Gmtd',
  'clean funk guitar': 'Gcln',
  'phaser guitar': 'Gph',
  'chorus guitar': 'Gch',
  'clav stabs': 'Clv',
  'rhodes EP': 'Rhds',
  ...
  'grand piano': 'Pno',
  'upright piano': 'Upno',
  'analog synth bass': 'Bsyn',
  'Moog mono bass': 'Bmoog',
  'sub bass': 'Bsub',
  'slap bass': 'Bslap',
  'finger bass': 'Bfin',
  'pick bass': 'Bpick',
  ...
  'snappy snare': 'Ssnap',
  'crack snare': 'Scrk',
  'swung 16th hats': 'HHsw16',
  'straight 16th hats': 'HH16',
  'tape saturation': 'tape70s',
  'light plate reverb': 'lpl',
  'spring reverb': 'spr',
  phaser: 'phs',
  chorus: 'cho',
  'auto-wah': 'awah',
...
const VARIATION_CURVES = [
  { id: 'layer', label: 'Layer Up' },
  { id: 'swap', label: 'Swap Colors' },
  { id: 'filter-rise', label: 'Filter Rise' },
  { id: 'fx-mutate', label: 'FX Mutate' },
] as const;
...
interface ReinstrumentState {
  hookBars: 2 | 4 | 8;
  repeats: number;
  curve: (typeof VARIATION_CURVES)[number]['id'];
  preset?: keyof typeof REINS_PRESETS;
}
interface VCState {
  verseBars: number;
  chorusBars: number;
...
interface ArrangementState {
  mode: (typeof ARR_MODES)[number]['id'];
  reinstrument: ReinstrumentState;
  vc: VCState;
  aaba: AABAState;
  build: BuildState;
}
...
function secondsFromBars(bpm: number, bars: number, meterTop = 4) {
  return ((bars * meterTop) / bpm) * 60;
}
...
function composeReadable(o: PromptState) {
  ...
  switch (o.arrangement.mode) {
    case 'reinstrument': {
      const a = o.arrangement.reinstrument;
      body.push(`single hook ${a.hookBars} bars repeats ${a.repeats} evolving (${a.curve})`);
      break;
    }
    case 'vc': {
      const v = o.arrangement.vc;
      body.push(
        `verse ${v.verseBars} / chorus ${v.chorusBars} pattern x${v.cycles}${v.addBridge ? ` + bridge ${v.bridgeBars}` : ''}`
      );
      break;
    }
    case 'aaba': {
      const a = o.arrangement.aaba;
      body.push(`A${a.aBars}-A${a.aBars}-B${a.bBars}-A${a.aBars}${a.cycles > 1 ? ` x${a.cycles}` : ''}`);
      break;
    }
    case 'build': {
      const b = o.arrangement.build;
      body.push(
        `intro ${b.introBars} → build ${b.buildBars} → drop ${b.dropBars} → post ${b.postBars} (${b.rises} rises)`
      );
      break;
    }
  }
...
function compressToken(t: string) {
  return (CODEMAP[t] || t)
    .replace(/analog/gi, 'anlg')
    .replace(/vintage/gi, 'vtg')
    .replace(/cosmic/gi, 'csm')
    .replace(/groovy/gi, 'grv')
    .replace(/warm/gi, 'wrm')
    .replace(/punchy/gi, 'pchy')
    .replace(/(with|and)/gi, '');
}
...
function composeCompressed(o: PromptState) {
  ...
  switch (o.arrangement.mode) {
    case 'reinstrument': {
      const a = o.arrangement.reinstrument;
      arrTokens.push(`H${a.hookBars}x${a.repeats}`, a.curve.replace(/-.*/, 'R'));
      break;
    }
    case 'vc': {
      const v = o.arrangement.vc;
      arrTokens.push(`V${v.verseBars}C${v.chorusBars}x${v.cycles}`);
      if (v.addBridge) arrTokens.push(`Br${v.bridgeBars}`);
      break;
    }
    case 'aaba': {
      const a = o.arrangement.aaba;
      arrTokens.push(`A${a.aBars}A${a.aBars}B${a.bBars}A${a.aBars}`);
      if (a.cycles > 1) arrTokens.push(`x${a.cycles}`);
      break;
    }
    case 'build': {
      const b = o.arrangement.build;
      arrTokens.push(`I${b.introBars}Bld${b.buildBars}D${b.dropBars}P${b.postBars}`, `R${b.rises}`);
      break;
    }
  }
  ...
  const instRank = (inst: string) => {
    ...
    for (const key in prioMap) {
      if (k.includes(key)) return prioMap[key];
    }
  ...
  for (const tok of candidates) {
    ...
    if (test.length > 200) {
      // remove last added and stop adding further lower-priority tokens
      outTokens.pop();
      break;
    }
  }
...
function calcRepeats(bpm: number, targetSec: number, hookBars: number, meterTop = 4) {
  const perLoop = secondsFromBars(bpm, hookBars, meterTop);
  return clamp(Math.round(targetSec / perLoop), 4, 40);
}
...
export default function FunkNaturalPage() {
  ...
  const [state, setState] = useState<PromptState>(() => {
    ...
    s.arrangement.reinstrument.repeats = calcRepeats(
      s.bpm,
      s.targetSec,
      s.arrangement.reinstrument.hookBars,
      s.meterTop
    );
    return s;
  });
  ...
  const predictedSec = useMemo(() => {
    switch (state.arrangement.mode) {
      case 'reinstrument': {
        const a = state.arrangement.reinstrument;
        return secondsFromBars(state.bpm, a.hookBars * a.repeats, state.meterTop);
      }
      case 'vc':
        return secondsFromBars(state.bpm, vcTotalBars(state.arrangement.vc), state.meterTop);
      case 'aaba':
        return secondsFromBars(state.bpm, aabaTotalBars(state.arrangement.aaba), state.meterTop);
      case 'build':
        return secondsFromBars(state.bpm, buildTotalBars(state.arrangement.build), state.meterTop);
      default:
        return secondsFromBars(state.bpm, 64, state.meterTop);
    }
  }, [state]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled && found) {
          setAnalysisEntry(found);
        }
        if (!cancelled && patchObj && patchObj.patch) {
          const lines = String(patchObj.patch)
            .split(/;|\n/)
            .map(l => l.trim())
            .filter(l => l.length > 0);
    ...
    async function doHash() {
      try {
        ...
        for (let i = 0; i < finalPrompt.length; i++) {
          h = (h * 31 + finalPrompt.charCodeAt(i)) >>> 0;
        }
  ...
  function logOffline() {
    ...
    try {
      ...
      setLogError(e.message || 'offline log 실패');
    } finally {
      setLogging(false);
    }
  }
  ...
  const setTarget = (sec: number) =>
    setState(s => {
      const next = { ...s, targetSec: clamp(sec, 90, 480) };
      if (next.arrangement.mode === 'reinstrument') {
        const a = { ...next.arrangement.reinstrument };
        a.repeats = calcRepeats(next.bpm, next.targetSec, a.hookBars, next.meterTop);
        next.arrangement = { ...next.arrangement, reinstrument: a };
      }
      return next;
    });
  const setHookBars = (bars: 2 | 4 | 8) =>
    setState(s => {
      const a = { ...s.arrangement.reinstrument, hookBars: bars };
      a.repeats = calcRepeats(s.bpm, s.targetSec, bars, s.meterTop);
      return { ...s, arrangement: { ...s.arrangement, reinstrument: a } };
    });
  // VC 편집 핸들러
  const updateVC = (patch: Partial<VCState>) =>
    setState(s => ({ ...s, arrangement: { ...s.arrangement, vc: { ...s.arrangement.vc, ...patch } } }));
  const updateAABA = (patch: Partial<AABAState>) =>
    setState(s => ({ ...s, arrangement: { ...s.arrangement, aaba: { ...s.arrangement.aaba, ...patch } } }));
  const updateBuild = (patch: Partial<BuildState>) =>
    setState(s => ({ ...s, arrangement: { ...s.arrangement, build: { ...s.arrangement.build, ...patch } } }));
  ...
  const Chip = ({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) => (
    <button
      onClick={onClick}
      className={`px-4 py-3 rounded-2xl text-sm md:text-base font-medium border transition ${active ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg' : 'bg-neutral-800/40 text-neutral-100 border-neutral-700 hover:border-neutral-500'}`}
    >
      {label}
    </button>
  );
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div className="space-y-4">
      <h3 className="text-lg md:text-xl font-semibold text-neutral-100">{title}</h3>
      {children}
    </div>
  );

  return (
    <div className="min-h-screen w-full funk-bg text-neutral-50">
      <div className="max-w-5xl mx-auto px-4 py-8 md:py-10 relative">
        <h1 className="text-2xl md:text-3xl font-bold mb-6 bg-gradient-to-r from-indigo-300 via-amber-200 to-cyan-300 bg-clip-text text-transparent">
          Funk Prompt Wizard v2
        </h1>
        <div className="flex gap-2 mb-8 flex-wrap">
          {[1, 2, 3, 4, 5].map(n => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm border ${step === n ? 'bg-indigo-600 border-indigo-500' : 'bg-neutral-900 border-neutral-700'}`}
            >
              Step {n}
            </button>
          ))}
        ...
        {step === 1 && (
          <div className="space-y-8">
              ...
              <div className="flex flex-wrap gap-3">
                {SUBSTYLES.map(s => (
                  <Chip
                    key={s}
                    label={s}
                    active={state.substyles.includes(s)}
                    onClick={() => setState(st => ({ ...st, substyles: toggleChip(st.substyles, s) }))}
                  />
                ))}
              </div>
            </Section>
              ...
              <div className="grid lg:grid-cols-3 gap-8 items-start">
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {[180, 210, 230, 240, 270].map(sec => (
                      <Chip
                        key={sec}
                        active={state.targetSec === sec}
                        label={toMMSS(sec)}
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(['short', 'long'] as LengthMode[]).map(m => (
                      <Chip
                        key={m}
                        label={m === 'short' ? 'Short (≤200c)' : 'Long (Narrative)'}
                        active={state.lengthMode === m}
                        onClick={() => setState(s => ({ ...s, lengthMode: m }))}
                      />
                    ))}
                  </div>
                  <div className="mt-2 text-xs text-neutral-400 leading-relaxed">
                    Short: 200자 이내 고밀도 프롬프트 (압축). Long: 더 서술적인 형태(최대 표시 {charLimit}자 권장) —
                    저장 시 하나만 기록.
                  </div>
                </div>
              </div>
            ...
            <Section title="Era">
              <div className="flex flex-wrap gap-3">
                {ERAS.map(t => (
                  <Chip key={t} label={t} active={state.era === t} onClick={() => setState(s => ({ ...s, era: t }))} />
                ))}
              </div>
            </Section>
            <div className="flex justify-end">
              <button className="px-5 py-3 rounded-2xl bg-indigo-600" onClick={() => setStep(2)}>
                다음 →
              </button>
            </div>
          </div>
        ...
        {step === 2 && (
          <div className="space-y-8">
                ...
                <div className="flex flex-wrap gap-3">
                  {items.map(i => (
                    <Chip
                      key={i}
                      label={i}
                      active={state.instruments.includes(i)}
                      onClick={() => setState(s => ({ ...s, instruments: toggleChip(s.instruments, i) }))}
                    />
                  ))}
                </div>
              </Section>
            ...
            <div className="flex justify-between">
              <button
                className="px-5 py-3 rounded-2xl bg-neutral-800 border border-neutral-700"
                onClick={() => setStep(1)}
              >
                ← 이전
              </button>
              <button className="px-5 py-3 rounded-2xl bg-indigo-600" onClick={() => setStep(3)}>
                다음 →
              </button>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <Section title="Arrangement Mode">
              <p className="text-xs text-neutral-400 leading-relaxed mb-3">
                원하는 전개 방식을 선택하세요. 각 모드는 곡의 에너지 흐름과 반복 패턴을 다른 방식으로 보여줍니다.
                (한국어 설명: 위 칩에 마우스를 올리거나 선택해 세부 조정)
              </p>
              <div className="flex flex-wrap gap-3">
                {ARR_MODES.map(m => (
                  <Chip
                    key={m.id}
                    label={m.label}
                    active={state.arrangement.mode === m.id}
                    onClick={() => setState(s => ({ ...s, arrangement: { ...s.arrangement, mode: m.id } }))}
                  />
                ))}
              </div>
            </Section>
            {state.arrangement.mode === 'reinstrument' && (
              <Section title="Reinstrumented Hook">
                <p className="text-xs text-neutral-400 mb-4">
                  단일 Hook 구간을 여러 번 반복하며 악기를 점진적으로 추가/교체하는 구조입니다. 짧은 Hook을 반복하면서
                  미묘한 진화감을 강조할 때 효과적입니다.
                </p>
                <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {[2, 4, 8].map(b => (
                        <Chip
                          key={b}
                          label={`${b} bars`}
                          active={state.arrangement.reinstrument.hookBars === b}
                          onClick={() => setHookBars(b as 2 | 4 | 8)}
                        />
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {VARIATION_CURVES.map(v => (
                        <Chip
                          key={v.id}
                          label={v.label}
                          active={state.arrangement.reinstrument.curve === v.id}
                          onClick={() =>
                            setState(s => ({
                              ...s,
                              arrangement: {
                                ...s.arrangement,
                                reinstrument: { ...s.arrangement.reinstrument, curve: v.id },
                              },
                            }))
                          }
                        />
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {Object.keys(REINS_PRESETS).map(k => (
                        <Chip
                          key={k}
                          label={k}
                          active={state.arrangement.reinstrument.preset === k}
                          onClick={() =>
                            setState(s => ({
                              ...s,
                              arrangement: {
                                ...s.arrangement,
                                reinstrument: { ...s.arrangement.reinstrument, preset: k as any },
                              },
                            }))
                          }
                        />
            ...
            {state.arrangement.mode === 'vc' && (
              <Section title="Verse / Chorus">
                <p className="text-xs text-neutral-400 mb-4">
                  Verse(도입/서술)와 Chorus(후렴/핵심 후킹)가 교차 반복되는 가장 보편적인 구조입니다. Bridge를 추가해
                  대비감을 줄 수 있습니다.
                </p>
                <div className="grid md:grid-cols-4 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      min={4}
                      max={32}
                      value={state.arrangement.vc.verseBars}
                      onChange={e => updateVC({ verseBars: Number(e.target.value) || 8 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      min={4}
                      max={32}
                      value={state.arrangement.vc.chorusBars}
                      onChange={e => updateVC({ chorusBars: Number(e.target.value) || 8 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      min={2}
                      max={10}
                      value={state.arrangement.vc.cycles}
                      onChange={e => updateVC({ cycles: Number(e.target.value) || 4 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 items-center">
                      <input
                        type="checkbox"
                        checked={state.arrangement.vc.addBridge}
                        onChange={e => updateVC({ addBridge: e.target.checked })}
                      />
                      <span className="text-xs">사용</span>
                    </div>
                    {state.arrangement.vc.addBridge && (
                      <input
                        type="number"
                        min={4}
                        max={32}
                        value={state.arrangement.vc.bridgeBars}
                        onChange={e => updateVC({ bridgeBars: Number(e.target.value) || 8 })}
                        className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                      />
                    )}
                  </div>
            ...
            {state.arrangement.mode === 'aaba' && (
              <Section title="AABA">
                <p className="text-xs text-neutral-400 mb-4">
                  고전적인 A-A-B-A 포맷. A는 주제(Theme), B는 대비(Bridge/Release) 섹션입니다. cycles를 늘리면 블록
                  자체를 반복합니다.
                </p>
                <div className="grid md:grid-cols-3 gap-6">
                  <div>
                    <label className="block text-xs opacity-70 mb-1">A Bars</label>
                    <input
                      type="number"
                      min={4}
                      max={32}
                      value={state.arrangement.aaba.aBars}
                      onChange={e => updateAABA({ aBars: Number(e.target.value) || 8 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-xs opacity-70 mb-1">B Bars</label>
                    <input
                      type="number"
                      min={4}
                      max={32}
                      value={state.arrangement.aaba.bBars}
                      onChange={e => updateAABA({ bBars: Number(e.target.value) || 8 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      min={1}
                      max={6}
                      value={state.arrangement.aaba.cycles}
                      onChange={e => updateAABA({ cycles: Number(e.target.value) || 1 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                </div>
            ...
            {state.arrangement.mode === 'build' && (
              <Section title="Build & Drop">
                <p className="text-xs text-neutral-400 mb-4">
                  점진적으로 에너지를 쌓아 올린 뒤(Dynamic Build) 강한 Drop을 만들고, Post 섹션에서 여운/변형 혹은 절반
                  에너지로 마무리하는 구조입니다.
                </p>
                <div className="grid md:grid-cols-5 gap-4">
                  <div>
                    <label className="block text-xs opacity-70 mb-1">Intro</label>
                    <input
                      type="number"
                      min={2}
                      max={32}
                      value={state.arrangement.build.introBars}
                      onChange={e => updateBuild({ introBars: Number(e.target.value) || 8 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-xs opacity-70 mb-1">Build</label>
                    <input
                      type="number"
                      min={4}
                      max={64}
                      value={state.arrangement.build.buildBars}
                      onChange={e => updateBuild({ buildBars: Number(e.target.value) || 16 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-xs opacity-70 mb-1">Drop</label>
                    <input
                      type="number"
                      min={4}
                      max={64}
                      value={state.arrangement.build.dropBars}
                      onChange={e => updateBuild({ dropBars: Number(e.target.value) || 16 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-xs opacity-70 mb-1">Post</label>
                    <input
                      type="number"
                      min={2}
                      max={32}
                      value={state.arrangement.build.postBars}
                      onChange={e => updateBuild({ postBars: Number(e.target.value) || 8 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-xs opacity-70 mb-1">Rises</label>
                    <input
                      type="number"
                      min={0}
                      max={6}
                      value={state.arrangement.build.rises}
                      onChange={e => updateBuild({ rises: Number(e.target.value) || 0 })}
                      className="bg-neutral-900 border border-neutral-700 rounded px-2 py-2 w-full text-sm"
                    />
                  </div>
                </div>
            ...
            <div className="flex justify-between">
              ...
              <button className="px-5 py-3 rounded-2xl bg-indigo-600" onClick={() => setStep(4)}>
                다음 →
              </button>
            </div>
          </div>
        ...
        {step === 4 && (
          <div className="space-y-8">
              ...
              <div className="flex flex-wrap gap-3">
                {MOODS.map(m => (
                  <Chip
                    key={m}
                    label={m}
                    active={state.mood.includes(m)}
                    onClick={() => setState(s => ({ ...s, mood: toggleChip(s.mood, m) }))}
                  />
                ))}
              </div>
            </Section>
              ...
              <div className="flex flex-wrap gap-3">
                {FX.map(f => (
                  <Chip
                    key={f}
                    label={f}
                    active={state.fx.includes(f)}
                    onClick={() => setState(s => ({ ...s, fx: toggleChip(s.fx, f) }))}
                  />
                ))}
              </div>
            </Section>
            <div className="flex justify-between">
              ...
              <button className="px-5 py-3 rounded-2xl bg-indigo-600" onClick={() => setStep(5)}>
                결과 보기 →
              </button>
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2 bg-neutral-900/70 border border-neutral-800 rounded-2xl p-5">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="px-2 py-1 rounded bg-indigo-600 text-xs font-semibold">
                    {state.lengthMode.toUpperCase()}
                  </span>
                </div>
                <div className="text-xs text-neutral-400 mb-3 leading-relaxed">
                  {state.lengthMode === 'short'
                    ? '고밀도 단문(≤200자). 아래 내용이 그대로 저장됩니다.'
                    : '서술형 확장 프롬프트. 연구/분석용 풍부한 정보 포함.'}
                </div>
                <div className="text-sm whitespace-pre-wrap break-words font-medium tracking-tight prompt-glow">
                  {finalPrompt}
                </div>
                <div className={`mt-3 text-xs ${finalPrompt.length > charLimit ? 'text-red-400' : 'text-emerald-400'}`}>
                  {finalPrompt.length} / {charLimit}
                  {state.lengthMode === 'long' && finalPrompt.length > charLimit ? ' (권장 초과)' : ''}
                </div>
                <div className="mt-4 flex gap-3 flex-wrap">
                  <button className="px-4 py-2 rounded-xl bg-indigo-600" onClick={() => copy(finalPrompt)}>
                    프롬프트 복사
                  </button>
                  {state.lengthMode === 'short' && (
                    <button
                      className="px-4 py-2 rounded-xl bg-neutral-800 border border-neutral-700"
                      onClick={() => copy(readable)}
                    >
                      Readable 보기/복사
                    </button>
                  )}
                  <button
                    className="px-4 py-2 rounded-xl bg-neutral-700 border border-neutral-600"
                    onClick={() => copy(filenamePrefix)}
                  >
                    파일 Prefix 복사
                  </button>
                  <button
                    className="px-4 py-2 rounded-xl bg-neutral-700 border border-neutral-600"
                    onClick={() => copy(colabSnippet)}
                  >
                    Colab 스니펫 복사
                  </button>
                  <button
                    disabled={logging}
                    className={`px-4 py-2 rounded-xl ${serverFilenamePrefix ? 'bg-emerald-600' : 'bg-fuchsia-600'} disabled:opacity-50`}
                    onClick={logOffline}
                  >
                    {logging ? '저장 중...' : serverFilenamePrefix ? '로컬 기록 완료 ✅' : '로컬 기록 (Offline)'}
                  </button>
                  <button
                    className="px-4 py-2 rounded-xl bg-neutral-700 border border-neutral-600"
                    onClick={exportQueue}
                  ...
                  <button
                    className="px-4 py-2 rounded-xl bg-neutral-900 border border-neutral-700"
                ...
                {serverFilenamePrefix && (
                  <div className="mt-2 text-xs text-emerald-400">
                    Local @ {new Date(loggedTs).toLocaleTimeString()} / hash {serverHash}
                  </div>
                )}
                {logError && <div className="mt-2 text-xs text-red-400">오류: {logError}</div>}
                {queueOpen && (
                  <div className="mt-4 p-3 bg-neutral-900/60 border border-neutral-800 rounded-xl max-h-56 overflow-auto text-[11px] font-mono space-y-1">
                    {loadQueue().length === 0 && <div className="opacity-50">(큐 비어 있음)</div>}
                    {loadQueue()
                      .slice()
                      .reverse()
                      .map(p => (
                        <div key={p.hash} className="border-b border-neutral-800 pb-1 mb-1">
                          <div className="text-emerald-400">{p.hash}</div>
                          <div className="truncate">{p.text.slice(0, 140)}</div>
                          <div className="opacity-50">
                            {p.mode} · {p.bpm}bpm · {p.ts}
                          </div>
                        </div>
                      ))}
                ...
                <div className="mt-6 grid md:grid-cols-3 gap-4 text-xs">
                  ...
                  <div className="bg-neutral-900/60 border border-neutral-800 rounded-xl p-3 space-y-2">
                    ...
                    <pre className="whitespace-pre-wrap break-words text-[10px] leading-tight font-mono opacity-80 max-h-40 overflow-auto">
                      {colabSnippet}
                    </pre>
                  </div>
                </div>
                ...
                <div className="mt-8 bg-neutral-900/70 border border-neutral-800 rounded-2xl p-5">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    {!analysisLoading && !analysisError && analysisEntry && (
                      <span className="text-xs text-emerald-400">동기화됨</span>
                    )}
                    ...
                    <button
                      onClick={() => {
                        ...
                        setAnalysisEntry(null);
                        setAnalysisPatch([]);
                        setStep(5); // 동일 step 재트리거 위해 강제 hash 변화 없으니 직접 호출
                        (async () => {
                          const targetHash = serverHash || promptHash;
                          if (!targetHash) return;
                          setAnalysisLoading(true);
                          setAnalysisError('');
                          try {
                            ...
                            const found = summaryJson.find(r => r.hash === targetHash);
                            const patchObj = patchJson.find(r => r.hash === targetHash);
                            if (found) setAnalysisEntry(found);
                            else setAnalysisError('해시 분석 레코드 없음');
                            if (patchObj?.patch) {
                              setAnalysisPatch(
                                String(patchObj.patch)
                                  .split(/;|\n/)
                                  .map(l => l.trim())
                                  .filter(l => l)
                              );
                            ...
                            setAnalysisError(e.message || '로드 실패');
                          } finally {
                            setAnalysisLoading(false);
                          }
                        })();
                      }}
                      className="text-xs px-2 py-1 rounded bg-neutral-800 border border-neutral-700 hover:border-neutral-500"
                    >
                      새로고침
                    </button>
                  </div>
                  {analysisError && <div className="text-xs text-red-400 mb-2">{analysisError}</div>}
                  {!analysisError && !analysisEntry && !analysisLoading && (
                    <div className="text-xs opacity-60">분석 레코드를 찾지 못했습니다. (Colab 결과 push 대기)</div>
                  )}
                  {analysisEntry && (
                    <div className="space-y-4">
                      <div className="grid md:grid-cols-4 gap-4 text-xs">
                        ...
                        <div className="bg-neutral-950/50 border border-neutral-800 rounded-lg p-3 space-y-1">
                          ...
                          <div className="text-[10px] opacity-60">
                            목표 {state.bpm} bpm | Δ{' '}
                            {analysisEntry.tempo_librosa
                              ? Math.abs(analysisEntry.tempo_librosa - state.bpm).toFixed(2)
                              : '--'}
                          </div>
                        </div>
                        <div className="bg-neutral-950/50 border border-neutral-800 rounded-lg p-3 space-y-1">
                          ...
                          <div className="font-semibold">
                            {analysisEntry.lufs ? analysisEntry.lufs.toFixed(2) : '--'}
                          </div>
                        ...
                        <div className="bg-neutral-950/50 border border-neutral-800 rounded-lg p-3 space-y-1">
                          ...
                          <div className="font-semibold">
                            {analysisEntry.spectral_centroid ? Math.round(analysisEntry.spectral_centroid) : '--'}
                          </div>
                          <div className="text-[10px] opacity-60">(밸런스 감시)</div>
                        </div>
                      </div>
                      {analysisPatch.length > 0 && (
                        <div className="mt-2">
                          <div className="text-xs font-semibold mb-1">다음 개선 패치 제안</div>
                          <ul className="list-disc pl-5 space-y-1 text-[11px]">
                            {analysisPatch.map((p, i) => (
                      ...
                      {analysisPatch.length === 0 && (
                        <div className="text-[11px] opacity-60">패치 제안 없음 또는 안정 범위 내</div>
                      )}
                      <div className="text-[10px] opacity-50 pt-2 border-t border-neutral-800">
                        Colab 기반 지표. 새 오디오 업로드 & 재분석 후 git push 하면 갱신.
                      </div>
                    </div>
                  )}
                </div>
                <details className="mt-5 group">
                  <summary className="cursor-pointer text-xs opacity-60 hover:opacity-100 transition">
                    디버그 / 내부 형태
                  </summary>
