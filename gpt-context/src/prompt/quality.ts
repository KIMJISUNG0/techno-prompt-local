...
export function evaluateDraft(draft: DraftStructure): QualityReport {
  ...
  Object.entries(freq).forEach(([w, c]) => {
    if (c > 4)
      issues.push({
        code: 'repeat-adj',
        message: `Adjective "${w}" appears ${c} times – consider variety.`,
        severity: 'warn',
      });
  });
  ...
  if (lowHits < 3)
    issues.push({
      code: 'lowend-scarce',
      message: 'Few low-end descriptors; consider clarifying bass / kick character.',
      severity: 'info',
    });
  // Adjective density
  const density = adj.length / Math.max(1, words.length);
  if (density > 0.5)
    issues.push({ code: 'adj-heavy', message: 'Descriptor density high; may risk model dilution.', severity: 'warn' });
  const uniqueAdjectives = Object.keys(freq).length;
  // Simple score: start 100 - penalties
  let score = 100;
  issues.forEach(i => {
    score -= i.severity === 'warn' ? 8 : 3;
  });
