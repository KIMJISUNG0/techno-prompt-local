...
export function exportPrompt(draft: DraftStructure, opts: PromptExportOptions = {}): string {
  ...
  const focusSec =
    draft.sections.find(s => s.kind === 'drop') || draft.sections.find(s => s.kind === 'main') || draft.sections[0];
...
function summarizeEnergy(draft: DraftStructure): string {
  ...
  const peakSec =
    draft.sections.find(s => s.kind === 'drop') ||
    draft.sections.find(s => s.energy === 5) ||
    draft.sections[Math.floor(draft.sections.length / 2)];
