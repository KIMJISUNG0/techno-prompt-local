...
class LiveEngine {
  ...
  private async initWorklet() {
    ...
    this.workletReady = (async () => {
      try {
        ...
        const node = new AudioWorkletNode(this.ctx!, 'bandlimited-osc', {
          numberOfOutputs: 1,
  ...
  private setupBuses() {
    ...
    this.ppLeftDelay = ctx.createDelay(2.0);
    this.ppRightDelay = ctx.createDelay(2.0);
    this.ppFeedback = ctx.createGain();
    this.ppFeedback.gain.value = 0.38;
    this.ppLeftPan = ctx.createStereoPanner();
    this.ppLeftPan.pan.value = -0.85;
    this.ppRightPan = ctx.createStereoPanner();
    this.ppRightPan.pan.value = 0.85;
          ...
          if (vel > 0) {
            ...
            if (step.locks && Object.keys(step.locks).length) {
              ...
              if (step.locks.cutoff != null) {
                clone.filter = {
                  ...(clone.filter || { type: 'lowpass', cutoff: step.locks.cutoff }),
                  cutoff: step.locks.cutoff,
                } as any;
              }
  ...
  private triggerWithOpts(ts: TrackState, time: number, hitIndex: number, vel: number, opts: PlayOptions) {
    ...
    if (role === 'kick') this.playKick(time, opts, vel);
      ...
      const note = this.pickNote(opts, hitIndex);
      if (note != null) this.playBass(time, opts, note, vel);
    } else if (role === 'pad') {
      const note = this.pickNote(opts, hitIndex) ?? 48;
      this.playPad(time, opts, note, vel);
    } else if (role === 'lead') {
      const note = this.pickNote(opts, hitIndex) ?? 60;
      this.playLead(time, opts, note, vel);
    } else if (role === 'guitar') {
      const note = this.pickNote(opts, hitIndex) ?? 52;
      this.playGuitar(time, opts, note, vel);
    } else if (role === 'bassGtr') {
      const note = this.pickNote(opts, hitIndex) ?? 36;
      this.playBassGuitar(time, opts, note, vel);
    } else if (role === 'piano') {
      const note = this.pickNote(opts, hitIndex) ?? 60;
      this.playPiano(time, opts, note, vel);
    } else if (role === 'organ') {
      const note = this.pickNote(opts, hitIndex) ?? 60;
      this.playOrgan(time, opts, note, vel);
    } else if (role === 'tom') {
      this.playTom(time, opts, vel);
    } else if (role === 'clap') {
      this.playClap(time, opts, vel);
    } else if (role === 'ride') {
      this.playRide(time, opts, vel);
      ...
      if (m[2]) {
        m[2].split(/\s*,\s*/).forEach(seg => {
          ...
          if (k === 'v') {
            ...
            if (!isNaN(num)) step.v = num;
          } else if (k === 'p') {
            const num = parseFloat(vStr);
            if (!isNaN(num)) step.p = Math.min(1, Math.max(0, num));
          } else if (k === 'dt') {
            const num = parseFloat(vStr);
            if (!isNaN(num)) step.dt = num / 1000;
          } else {
  ...
  private playKick(time: number, opts: PlayOptions, vel: number) {
    ...
    bodyEQ.type = 'peaking';
    bodyEQ.frequency.value = 80;
    bodyEQ.Q.value = 1.2;
    bodyEQ.gain.value = 3.5;
    ...
    nGain.gain.setValueAtTime(0.4 * vel, time);
  ...
  private playBass(time: number, opts: PlayOptions, note: number, vel: number) {
    ...
    if (this.useHQ && (opts.wave === 'saw' || opts.wave === 'square' || opts.wave === 'supersaw')) {
      this.sendHQVoices(
        ...
        [
          {
            id: 'b' + ++this.wlIdCounter,
            freq,
            gain: (opts.gain ?? 0.6) * vel,
            wave: 'saw',
            env: { attack: 0.005, decay: decay * 0.6, sustain: 0.4, release: decay * 0.4 },
          },
        ],
        opts,
        outBus
    ...
    lp.type = 'lowpass';
    lp.frequency.setValueAtTime(780, time);
    // supersaw option
  ...
  private playSnare(time: number, opts: PlayOptions, vel: number) {
    ...
    bp.Q.value = 1.2;
    const nGain = ctx.createGain();
    nGain.gain.setValueAtTime((opts.gain ?? 0.8) * vel, time);
    nGain.gain.exponentialRampToValueAtTime(0.0001, time + noiseDur);
    nSrc.connect(bp).connect(nGain).connect(this.masterGain!);
    nSrc.start(time);
    // tone
    const osc = ctx.createOscillator();
    osc.type = 'triangle';
    ...
    oGain.gain.setValueAtTime(0.3 * vel, time);
    oGain.gain.exponentialRampToValueAtTime(0.0001, time + 0.25);
  ...
  private playPad(time: number, opts: PlayOptions, note: number, vel: number) {
    ...
    if (this.useHQ && (opts.wave === 'saw' || opts.wave === 'square' || opts.wave === 'supersaw')) {
      ...
      for (let i = 0; i < unison; i++) {
        ...
        voices.push({
          id: 'p' + ++this.wlIdCounter + '_' + i,
          freq: freq * Math.pow(2, cents / 1200),
          gain: ((opts.gain ?? 0.5) * vel) / unison,
          wave: 'saw',
          env: { attack: 0.35, decay: 0.7, sustain: 0.65, release: 2.2 },
        });
      }
    ...
    if (opts.wave === 'supersaw') {
      ...
      for (let i = 0; i < voices; i++) {
        ...
        o.start(time);
        o.stop(time + attack + decay + rel + 3);
      }
    } else {
      const o = ctx.createOscillator();
      const wt =
  ...
  private playLead(time: number, opts: PlayOptions, note: number, vel: number) {
    ...
    if (this.useHQ && (opts.wave === 'saw' || opts.wave === 'square' || opts.wave === 'supersaw')) {
      ...
      for (let i = 0; i < unison; i++) {
        ...
        const cents = spread * det;
        voices.push({
          id: 'l' + ++this.wlIdCounter + '_' + i,
          freq: baseFreq * Math.pow(2, cents / 1200),
          gain: ((opts.gain ?? 0.55) * vel) / unison,
          wave: 'saw',
          env: { attack: 0.008, decay: 0.22, sustain: 0.45, release: 0.28 },
        });
      }
      this.sendHQVoices(time, voices, opts, outBus);
      return;
    ...
    if (opts.wave === 'supersaw') {
      ...
      for (let i = 0; i < unison; i++) {
        ...
        const g = ctx.createGain();
        g.gain.value = 1 / unison;
        osc.connect(g).connect(out);
        osc.start(time);
        osc.stop(time + attack + decay + rel + 0.6);
      }
    } else {
      const osc = ctx.createOscillator();
      const wt =
  ...
  private sendHQVoices(time: number, voices: any[], _opts: PlayOptions, _target: AudioNode) {
    ...
    voices.forEach(v => {
      this.wlPort!.postMessage({
        type: 'noteOn',
        id: v.id,
        freq: v.freq,
        gain: v.gain,
        wave: v.wave === 'square' ? 'square' : 'saw',
        attack: v.env.attack,
        decay: v.env.decay,
        sustain: v.env.sustain,
        release: v.env.release,
        time,
      });
      ...
      setTimeout(() => {
        if (this.wlPort) this.wlPort.postMessage({ type: 'noteOff', id: v.id, time: offTime });
      }, delayMs);
    });
  ...
  private routeTrack(opts: PlayOptions, _vel: number): AudioNode {
    ...
    let ts: TrackState | undefined;
    this.tracks.forEach(v => {
      if (v.opts === opts) ts = v;
    });
    if (ts && ts.duckGain) {
      if (!(ts.duckGain as any)._wired) {
        ts.duckGain.connect(this.masterGain!);
        (ts.duckGain as any)._wired = true;
      }
      return ts.duckGain;
    }
  ...
  private playGuitar(time: number, opts: PlayOptions, note: number, vel: number) {
    ...
    src.buffer = buf;
    const g = ctx.createGain();
    g.gain.setValueAtTime((opts.gain ?? 0.55) * vel, time);
  ...
  private playPiano(time: number, opts: PlayOptions, note: number, vel: number) {
    ...
    partials.forEach((p, _i) => {
      ...
      const g = ctx.createGain();
      g.gain.value = (1 / partials.length) * (1 / (_i + 1));
      o.connect(g).connect(out);
      o.start(time);
      o.stop(time + a + d + r + 0.4);
    });
    out.gain.setValueAtTime(0, time);
  ...
  private playOrgan(time: number, opts: PlayOptions, note: number, vel: number) {
    ...
    out.gain.value = 0;
    out.connect(this.masterGain!);
    const a = opts.env?.attack ?? 0.02;
    const r = opts.env?.release ?? 0.9;
    const s = opts.env?.sustain ?? 0.95;
    const d = opts.env?.decay ?? 0.15;
    harmonics.forEach((h, _i) => {
      const o = ctx.createOscillator();
      o.type = 'sine';
      o.frequency.setValueAtTime(base * h, time);
      const g = ctx.createGain();
      g.gain.value = 1 / harmonics.length;
      o.connect(g).connect(out);
      o.start(time);
      o.stop(time + a + d + r + 1.2);
    });
    out.gain.setValueAtTime(0, time);
    out.gain.linearRampToValueAtTime((opts.gain ?? 0.5) * vel, time + a);
    out.gain.linearRampToValueAtTime((opts.gain ?? 0.5) * vel * s, time + a + d);
    out.gain.linearRampToValueAtTime(0.0001, time + a + d + r);
  }
  ...
  private playTom(time: number, opts: PlayOptions, vel: number) {
    ...
    osc.frequency.exponentialRampToValueAtTime(base * 0.7, time + 0.25);
    const gain = ctx.createGain();
    gain.gain.setValueAtTime((opts.gain ?? 0.8) * vel, time);
    ...
    const nGain = ctx.createGain();
    nGain.gain.setValueAtTime(0.25 * vel, time);
    nGain.gain.exponentialRampToValueAtTime(0.0001, time + 0.1);
    osc.connect(gain).connect(this.masterGain!);
    nSrc.connect(nGain).connect(this.masterGain!);
    osc.start(time);
    osc.stop(time + 0.45);
    nSrc.start(time);
  }
  ...
  private playClap(time: number, opts: PlayOptions, vel: number) {
    const ctx = this.ctx!;
    const hits = [0, 0.012, 0.028, 0.06];
    hits.forEach((off, _i) => {
      const buf = ctx.createBuffer(1, ctx.sampleRate * 0.12, ctx.sampleRate);
      const data = buf.getChannelData(0);
      for (let j = 0; j < data.length; j++) data[j] = (Math.random() * 2 - 1) * Math.pow(1 - j / data.length, 2.5);
      const src = ctx.createBufferSource();
      src.buffer = buf;
      const g = ctx.createGain();
      const scale = (opts.gain ?? 0.7) * vel * (_i === 0 ? 1 : 0.7);
      g.gain.setValueAtTime(scale, time + off);
      g.gain.exponentialRampToValueAtTime(0.0001, time + off + 0.13);
      const hp = ctx.createBiquadFilter();
      hp.type = 'highpass';
  ...
  private playRide(time: number, opts: PlayOptions, vel: number) {
    ...
    const data = buf.getChannelData(0);
    for (let i = 0; i < data.length; i++) data[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / data.length, 1.1);
    const src = ctx.createBufferSource();
    src.buffer = buf;
    const bp = ctx.createBiquadFilter();
    bp.type = 'bandpass';
    bp.frequency.value = 7800;
    bp.Q.value = 0.6;
    const g = ctx.createGain();
    g.gain.setValueAtTime((opts.gain ?? 0.38) * vel, time);
...
export function getLiveAPI() {
  return {
    ...
    play: async (id: string, opts: PlayOptions) => {
      await liveEngine.startIfNeeded();
      liveEngine.play(id, opts);
    },
    ...
    registerPatch: (name: string, desc: any) => {
      (patchRegistry as any).register(name, desc);
    },
    triggerPatch: (name: string, opt?: any) => {
      (patchRegistry as any).trigger(name, opt);
    },
    ...
    setSaturation: (drive: number) => {
      (liveEngine as any).satDrive = Math.max(0, drive);
      (liveEngine as any).updateSaturationCurve?.();
    },
    setClipCeiling: (ceil: number) => {
      (liveEngine as any).clipCeiling = Math.min(0.999, Math.max(0.2, ceil));
      (liveEngine as any).updateClipCurve?.();
    },
    configureDelay: (opts: { time?: number; feedback?: number; mix?: number; pingpong?: boolean }) => {
      const eng: any = liveEngine;
      if (opts.time != null) {
        if (eng.delayNode) eng.delayNode.delayTime.value = opts.time;
        if (eng.ppLeftDelay) eng.ppLeftDelay.delayTime.value = opts.time;
        if (eng.ppRightDelay) eng.ppRightDelay.delayTime.value = opts.time;
      }
      if (opts.feedback != null) {
        if (eng.delayFeedback) eng.delayFeedback.gain.value = opts.feedback;
        if (eng.ppFeedback) eng.ppFeedback.gain.value = opts.feedback;
      }
      if (opts.mix != null) {
        if (eng.delaySend) eng.delaySend.gain.value = opts.mix;
      }
      if (opts.pingpong != null) {
        eng.usePingPong = !!opts.pingpong;
      }
    },
    setReverb: (mix: number) => {
      const eng: any = liveEngine;
      if (eng.reverbSend) eng.reverbSend.gain.value = mix;
    },
    setReverbDuck: (amount: number, release?: number) => {
      const eng: any = liveEngine;
      eng.reverbDuckAmount = Math.min(0.95, Math.max(0.05, amount));
      if (release != null) eng.reverbDuckRelease = Math.max(0.1, release);
    },
    getLoudness: () => {
      return { momentary: (liveEngine as any).momentaryLufs, integrated: (liveEngine as any).integratedLufs };
    },
    setStereoWidth: (w: number) => {
      (liveEngine as any).updateStereoWidth?.(w);
    },
    setLowMono: (freq: number) => {
      (liveEngine as any).updateLowMono?.(freq);
    },
...
class PatchRegistry {
  ...
  register(name: string, desc: PatchDescriptor) {
