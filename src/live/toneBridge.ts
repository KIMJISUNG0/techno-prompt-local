...
function buildFXChain(t: any, inst: any, fx: FXSpec | FXSpec[]) {
  ...
  for (const spec of list) {
    if (typeof spec === 'string') {
      ...
      if (
        (type === 'reverb' || type === 'delay' || type === 'distortion' || type === 'chorus') &&
        ...
        try {
          if (type === 'reverb') custom = new t.Reverb({ decay: spec.decay ?? 3, wet: spec.wet ?? 0.35 });
            ...
            custom = new t.Chorus(spec.frequency ?? 4, spec.delayTime ?? 2.5, spec.depth ?? 0.5).start();
          else if (type === 'bitcrusher') custom = new t.BitCrusher(spec.bits ?? 4);
          else if (type === 'phaser')
            custom = new t.Phaser({
              frequency: spec.frequency ?? 15,
              octaves: spec.octaves ?? 3,
              baseFrequency: spec.baseFrequency ?? 350,
            });
          ...
          console.warn('[toneBridge] custom FX build failed', err);
        }
      }
    }
  }
...
export async function tonePlay(id: string, opts: TonePlayOptions) {
  ...
  const { type = 'synth', notes = [], duration = '8n', velocity = 0.9, volume = 0, spread = 0, sustain, fx } = opts;
...
export async function toneStop(id: string) {
  ...
  try {
    ...
    else if (inst.releaseAll) inst.releaseAll();
  } catch {
...
export async function tonePatternPlay(
  id: string,
  pattern: string,
  opts?: { type?: string; velocity?: number; step?: string; fx?: any }
) {
  ...
  const runtime: PatternRuntime = {
    id,
    pattern,
    synthType: opts?.type || 'synth',
    baseVelocity: opts?.velocity ?? 0.9,
    events: tokens,
  };
  ...
  const loopId = t.Transport.scheduleRepeat(_time => {
    ...
    for (const ev of runtime.events) {
      ...
      if (ev.type === 'note') {
        ...
        try {
          tonePlay('pat:' + id + ':' + stepIndex, {
            type: runtime.synthType as any,
            notes: [ev.note],
            duration: toneDur,
            velocity: vel,
            fx: opts?.fx,
          ...
          console.warn('[tonePattern] play error', err);
        }
        stepIndex += durSteps;
      }
    }
...
export function tonePatternStop(id: string) {
  const r = patternMap.get(id);
  if (!r) return;
  patternMap.delete(id);
  if (r.loopId != null && tone) {
    try {
      tone.Transport.clear(r.loopId);
    } catch {
      /* ignore clear error */
    }
  }
  ...
  sweepTimer = window.setTimeout(() => {
    ...
    for (const [id, rec] of Object.entries(active)) {
      if (now - rec.lastUse > TTL_MS) {
        try {
          if (rec.inst.dispose) rec.inst.dispose();
        } catch {
          /* ignore dispose error */
        }
        delete active[id];
