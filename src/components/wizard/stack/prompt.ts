...
export function buildFinalPrompt(state: StackState) {
  ...
  const bass = state.layers.find(l => l.role === 'bass');
  if (bass) parts.push('BASS: ' + describeMelodic(bass));
  const chords = state.layers.find(l => l.role === 'chords');
  if (chords) parts.push('CHORDS: ' + describeMelodic(chords));
  const lead = state.layers.find(l => l.role === 'lead');
  if (lead) parts.push('LEAD: ' + describeMelodic(lead));
