...
export default function TestPlayground() {
  ...
  function start() {
    ...
    if (!api) return;
    api.setToneBPM?.(bpm);
    api.tonePatternPlay?.('testArp', pattern, {
      type: 'synth',
      velocity: 0.9,
      fx: [
        { type: 'reverb', decay: 3, wet: 0.35 },
        { type: 'delay', time: '8n', feedback: 0.3, wet: 0.25 },
      ],
    });
    setPlaying(true);
  }
  ...
  return (
    <div className="p-6 space-y-6 text-slate-200 font-mono">
      <h1 className="text-xl font-semibold tracking-widest bg-gradient-to-r from-cyan-400 via-fuchsia-400 to-violet-400 bg-clip-text text-transparent">
      ...
      <div className="flex flex-wrap gap-4 items-end">
        <label className="text-xs flex flex-col">
          ...
          <input
            ...
            className="bg-slate-800/60 border border-slate-600 rounded px-2 py-1 text-sm w-24"
          />
        </label>
        <label className="text-xs flex-1 min-w-[260px]">
          ...
          <input
            ...
            className="bg-slate-800/60 border border-slate-600 rounded px-2 py-1 text-sm w-full"
          />
        </label>
        {!playing && (
          <button
            onClick={start}
            className="px-4 py-2 rounded bg-cyan-500 text-slate-900 font-semibold text-xs tracking-wide hover:brightness-110"
          >
            Start
          </button>
        )}
        {playing && (
          <button
            onClick={stop}
            className="px-4 py-2 rounded bg-fuchsia-500 text-slate-900 font-semibold text-xs tracking-wide hover:brightness-110"
          >
            Stop
          </button>
        )}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <div className="text-[10px] h-40 overflow-auto bg-slate-900/50 border border-slate-700 rounded p-2 leading-relaxed whitespace-pre-wrap">
            {log.join('\n')}
          </div>
        </div>
        <div className="space-y-2">
      ...
      <div className="text-[10px] text-slate-500">
        Hash route: <code>#live-test</code>. Return:{' '}
        <a className="text-cyan-300 underline" href="#wizard">
          Wizard
        </a>
      </div>
    </div>
