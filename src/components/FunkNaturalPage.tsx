...
type Step = 1|2|3|4|5;
const ARR_MODES = [ {id:'reinstrument',label:'Reinstrumented Hook'}, {id:'vc',label:'Verse/Chorus'}, {id:'aaba',label:'AABA'}, {id:'build',label:'Build & Drop'} ] as const;
...
interface ArrangementState{ mode: typeof ARR_MODES[number]['id']; reinstrument: ReinstrumentState }
...
function composeReadable(o:PromptState){
  ...
  if(o.arrangement.mode==='reinstrument'){
    const a=o.arrangement.reinstrument; body.push(`hook ${a.hookBars} bars x${a.repeats} ${a.curve}`);
  }
...
function composeCompressed(o:PromptState){
  ...
  if(o.arrangement.mode==='reinstrument'){ const a=o.arrangement.reinstrument; arrTokens.push(`H${a.hookBars}x${a.repeats}`, a.curve.replace(/-.*/,'R')); }
...
const defaultState: PromptState = {
  ...
  arrangement:{ mode:'reinstrument', reinstrument:{ hookBars:4, repeats:0, curve:'layer', preset:'LAYER_FUNKEVO' } },
  targetSec:230
};
...
export default function FunkNaturalPage(){
  ...
  const predictedSec = useMemo(()=>{ if(state.arrangement.mode==='reinstrument'){ const a=state.arrangement.reinstrument; return secondsFromBars(state.bpm,a.hookBars*a.repeats,state.meterTop); } return secondsFromBars(state.bpm,64,state.meterTop); },[state]);
  ...
  return <div className="min-h-screen w-full bg-neutral-950 text-neutral-50"><div className="max-w-5xl mx-auto px-4 py-8 md:py-10"><h1 className="text-2xl md:text-3xl font-bold mb-6">Funk Prompt Wizard v2</h1>
    ...
    {step===3 && <div className="space-y-8"><Section title="Arrangement Mode"><div className="flex flex-wrap gap-3">{ARR_MODES.map(m=> <Chip key={m.id} label={m.label} active={state.arrangement.mode===m.id} onClick={()=> setState(s=> ({...s,arrangement:{...s.arrangement,mode:m.id}}))} />)}</div></Section>{state.arrangement.mode==='reinstrument' && <Section title="Reinstrumented Hook"><div className="grid md:grid-cols-3 gap-6"><div><label className="block mb-2 text-sm opacity-80">Hook Bars</label><div className="flex gap-2 flex-wrap">{[2,4,8].map(b=> <Chip key={b} label={`${b} bars`} active={state.arrangement.reinstrument.hookBars===b} onClick={()=> setHookBars(b as 2|4|8)} />)}</div></div><div><label className="block mb-2 text-sm opacity-80">Variation Curve</label><div className="flex gap-2 flex-wrap">{VARIATION_CURVES.map(v=> <Chip key={v.id} label={v.label} active={state.arrangement.reinstrument.curve===v.id} onClick={()=> setState(s=> ({...s,arrangement:{...s.arrangement,reinstrument:{...s.arrangement.reinstrument,curve:v.id}}}))} />)}</div></div><div><label className="block mb-2 text-sm opacity-80">Preset</label><div className="flex gap-2 flex-wrap">{Object.keys(REINS_PRESETS).map(k=> <Chip key={k} label={k} active={state.arrangement.reinstrument.preset===k} onClick={()=> setState(s=> ({...s,arrangement:{...s.arrangement,reinstrument:{...s.arrangement.reinstrument,preset:k as any}}}))} />)}</div></div></div><div className="mt-3 text-xs opacity-70">Hook 반복 수는 목표 길이에 맞게 자동 계산합니다.</div><div className="mt-1 text-lg font-semibold">Repeats: {state.arrangement.reinstrument.repeats}</div></Section>}<div className="flex justify-between"><button className="px-5 py-3 rounded-2xl bg-neutral-800 border border-neutral-700" onClick={()=> setStep(2)}>← 이전</button><button className="px-5 py-3 rounded-2xl bg-indigo-600" onClick={()=> setStep(4)}>다음 →</button></div></div>}
