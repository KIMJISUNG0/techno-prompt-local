...
function Chip({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`px-2 py-1 rounded-full border text-[11px] transition ${active ? 'border-cyan-400 text-cyan-200 bg-cyan-500/10' : 'border-slate-600 text-slate-300 hover:border-cyan-400 hover:text-cyan-200'}`}
    >
      {label}
    </button>
  );
...
export default function SchemaPromptBuilder({ schema, bpm, meter, swing, extraSuffix, compact }: Props) {
  const [sel, setSel] = useState<Record<string, Set<string>>>(
    () => Object.fromEntries(schema.groups.map(g => [g.id, new Set()])) as any
  );
  ...
  const prompt = useMemo(() => {
    ...
    for (const gid of schema.order) {
      ...
      const chunk = [...set]
        .map(id => optionIndex.get(id)?.prompt)
        .filter(Boolean)
        .join(', ');
    ...
    if (params.size) {
      ...
      for (const pid of params) {
        ...
        outer: for (const list of Object.values(suboptIndex)) {
          ...
          if (found) {
            paramPrompts.push(found.prompt);
            break outer;
          }
        }
      }
  ...
  return (
    <div className={compact ? 'space-y-4' : 'space-y-6'}>
      <div className={`rounded-xl border border-white/10 bg-white/5 ${compact ? 'p-3' : 'p-4'}`}>
        <h3 className="text-xs uppercase tracking-widest text-cyan-200 mb-2">
          Schema Builder (Universal + Genre)
          {compact && <span className="ml-2 text-[10px] text-slate-500">Compact</span>}
        </h3>
        <div className="flex flex-col gap-6">
          ...
          {allTags.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-2 items-center">
              <select
                ...
                className="text-[10px] px-1.5 py-0.5 rounded border border-slate-600 bg-slate-800/70 focus:outline-none focus:border-cyan-400"
              >
              ...
              {allTags.map(t => {
                ...
                return (
                  <button
                    key={t}
                    onClick={() => toggleTag(t)}
                    className={`px-2 py-0.5 rounded-full text-[10px] border transition ${on ? 'border-fuchsia-400 text-fuchsia-200 bg-fuchsia-600/20 shadow-[0_0_8px_rgba(217,70,239,0.5)]' : 'border-slate-600 text-slate-400 hover:border-cyan-400 hover:text-cyan-200'}`}
                  >
                    {t}
                  </button>
                );
              ...
              {tagFilters.size > 0 && (
                <button
                  ...
                  className="px-2 py-0.5 rounded-full text-[10px] border border-slate-500 text-slate-300 hover:border-cyan-400"
                >
                  Clear
                </button>
              )}
          ...
          {schema.order.map(gid => {
            ...
            const sorted = [...list].sort((a, b) => {
              const aSel = sel[gid]?.has(a.id) ? 1 : 0;
              const bSel = sel[gid]?.has(b.id) ? 1 : 0;
            ...
            const selLabels =
              selSet && selSet.size > 0
                ? ([...selSet].map(id => optionIndex.get(id)?.label).filter(Boolean) as string[])
                : [];
            const isCollapsed = compact && collapsed[gid];
            return (
              <div key={gid} className={compact ? 'space-y-1' : 'space-y-2'}>
                <div
                  className={`flex items-center gap-2 ${compact ? 'cursor-pointer select-none' : ''}`}
                  onClick={() => compact && toggleCollapse(gid)}
                >
                  {compact && (
                    <span className={`text-[9px] transition-transform ${isCollapsed ? 'rotate-0' : 'rotate-90'}`}>
                      ▶
                    </span>
                  )}
                  ...
                  {g.universal && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-cyan-500/10 border border-cyan-400/30 text-cyan-300">
                      UNI
                    </span>
                  )}
                  {compact && selLabels.length > 0 && (
                    <span className="text-[9px] text-cyan-300 truncate max-w-[160px]">
                      {selLabels.slice(0, 3).join(', ')}
                      {selLabels.length > 3 ? '…' : ''}
                    </span>
                  )}
                </div>
                    ...
                    const filteredByTag =
                      tagFilters.size === 0
                        ...
                        : sorted.filter(o => {
                            ...
                            if (tagMode === 'OR') {
                              for (const t of o.tags) if (tagFilters.has(t)) return true;
                              return false;
                            }
                            if (tagMode === 'AND') {
                              for (const ft of tagFilters) if (!o.tags.includes(ft)) return false;
                              return true;
                            }
                            // NOT mode: exclude any item having selected tag
                            for (const t of o.tags) if (tagFilters.has(t)) return false;
                            return true;
                          });
                    return (
                                ...
                                <div className="mt-1 flex flex-wrap gap-1">
                                  {subs.map(s => (
                                    <button
                                      key={s.id}
                                      onClick={() => addSub(s.id)}
                                      className="text-[9px] px-1.5 py-0.5 rounded border border-slate-600 hover:border-cyan-400 text-slate-400 hover:text-cyan-200"
                                    >
                                      {s.label}
                                    </button>
                                  ))}
      ...
      <div className={`rounded-xl border border-white/10 bg-black/40 ${compact ? 'p-3' : 'p-4'}`}>
        ...
        <div
          className={`min-h-[120px] text-sm leading-relaxed bg-black/30 rounded-lg ${compact ? 'p-2' : 'p-3'} border border-slate-700`}
        >
          {prompt || <span className="text-slate-500">Build a prompt…</span>}
        </div>
        <div className="mt-2 flex gap-2">
          <button
            onClick={() => navigator.clipboard.writeText(prompt)}
            className="px-3 py-1.5 rounded bg-cyan-600/30 border border-cyan-400/60 text-[11px] hover:bg-cyan-600/40"
          >
            Copy
          </button>
          <button
            onClick={() => {
              setSel(Object.fromEntries(schema.groups.map(g => [g.id, new Set()])) as any);
              setParams(new Set());
            }}
