...
(() => {
  try {
    ...
    /* ignore hash parse errors */ void 0;
  }
})();
...
function RootChooser() {
  ...
  if (!picked)
    return (
      <GenrePortal
        onPick={ids => {
          (window as any).__pickedGenres = ids;
          setPicked(ids);
        }}
      />
    );
...
function AppShell() {
  ...
  return (
    <div className={'min-h-screen flex flex-col ' + (mode === 'dark' ? 'app-dark-root' : 'bg-slate-100')}>
      <nav
        className={
          (mode === 'dark' ? 'app-dark-nav ' : 'bg-white/70 backdrop-blur border-slate-200 ') +
          'border-b flex gap-3 px-4 py-2 text-xs tracking-wide items-center'
        }
      >
        <button onClick={() => (window as any).resetGenre?.()} className="btn">
          Genres
        </button>
        <span className="text-slate-600">|</span>
        <div className="flex gap-2 items-center">
          <span className="text-slate-500">Mode:</span>
          <a href="#/expert" className="btn">
            Expert
          </a>
          <a href="#/abstract" className="btn">
            Abstract
          </a>
          <a href="#/unified" className="btn">
            Unified
          </a>
          <a href="#/funk-natural" className="btn">
            Funk Natural
          </a>
          <a href="#/composer" className="btn">
            Composer
          </a>
          <a href="#/live-test" className="btn">
            Live Test
          </a>
        </div>
        ...
        >
          <span className="tt-ico" aria-hidden>
            {mode === 'dark' ? '🌙' : '☀️'}
          </span>
      ...
      <div className="flex-1">
        <ErrorBoundary>
          <React.Suspense
            fallback={<div className="p-6 text-xs text-slate-500">Loading module… (느리면 새로고침 Ctrl+Shift+R)</div>}
          >
            <RootChooser />
          </React.Suspense>
        </ErrorBoundary>
...
class ErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean; err?: any }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="p-6 text-xs text-red-400 space-y-2">
          로딩 중 오류가 발생했습니다.
