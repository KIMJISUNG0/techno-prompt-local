...
export const PROG_CATEGORIES: ProgCategory[] = [
  ...
  },
  {
    ...
    nextHints() {
      return ['fx', 'arrangement'];
    },
  },
  {
    id: 'fx',
    label: 'FX & Transitions',
    min: 0,
    max: 4,
    tokens: [
      t('noise sweep'),
  ...
  // New extended dimension: space / ambience focus
  {
    ...
    tokens: [
      t('deep reverb tails'),
      t('short room slap'),
      t('dub delay feedback'),
      t('shimmer verb'),
      t('binaural pans'),
      t('reverse ambience'),
      t('vacuum silence drops'),
    ],
    nextHints() {
      return ['mood'];
    },
  ...
  // Vocal / human elements
  {
    ...
    max: 3,
    tokens: [
      t('ethereal vocal chops'),
      t('spoken word texture'),
      t('vocoded drone'),
      t('processed chant'),
      t('breathy phrase'),
      t('pitch shifted fragment'),
    ],
    nextHints() {
      return ['fx', 'mix'];
    },
  ...
  // Texture additive layer (mic / field / analog)
  {
    ...
    max: 5,
    tokens: [
      t('vinyl crackle loop'),
      t('tape hiss bed'),
      t('granular dust'),
      t('analog noise swell'),
      t('field recording city'),
      t('forest ambience'),
      t('machine hum layer'),
      t('filtered static pulses'),
    ],
    nextHints() {
      return ['mix', 'mood'];
    },
  ...
  {
    id: 'master',
    label: 'Mastering Intent',
    min: 0,
    max: 3,
    tokens: [
      t('club loudness'),
      t('dynamic headroom'),
      t('warm glue compression'),
      t('wide stereo polish'),
      t('vinyl master color'),
      t('streaming optimized'),
    ],
  },
  {
    id: 'mood',
    label: 'Final Mood Tags',
    min: 1,
    max: 4,
    tokens: [
      t('tense'),
